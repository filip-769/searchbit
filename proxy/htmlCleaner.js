...
export default function htmlCleaner (html, baseUrl) {
    ...
    dom.window.document.querySelectorAll("script, meta:not([http-equiv=refresh]), applet, frameset, frame, link:not([rel=alternate]):not([rel=author]):not([rel=help]):not([rel*=icon]):not([rel=licence]):not([rel=me]):not([rel=next]):not([rel=prev]):not([rel=shortlink]):not([rel=stylesheet])").forEach(el => el.remove());
    dom.window.document.querySelectorAll("[src]:not([src^='data:']), [href]:not([href^='#']), [srcset], [poster], [cite], [data], [background]").forEach(el => {
        try {
